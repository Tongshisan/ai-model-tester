...
export async function dbCreateChat(chat: Omit<Chat, 'id' | 'created_at'>): Promise<Chat> {
...
export async function dbUpdateChat(id: string, updates: Partial<Chat>): Promise<void> {
  if (!supabase) return;
...
export async function dbCreateMessage(message: Omit<Message, 'id' | 'created_at'>): Promise<Message> {
  if (!supabase) throw new Error('Supabase not configured');
  const { data, error } = await supabase
    .from('messages')
    .insert(message)
